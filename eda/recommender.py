...
def main():
    ...
    if option_2:
        ...
        with st.form('average_rating'):
            ...
            num_recommendations = input_col2.number_input('Number of Recommendations',min_value=1,max_value=1000, value=10,step=1)
            submitted = st.form_submit_button('Show Movies')
            if submitted and option=='Low->High':
                st.write(avg_movie_ratings.sort_values(by=['avg_rating'])[:num_recommendations])
    ...
    if option_6:
        with st.form('rec_movie_user'):
            ...
            num_recommendations = input_col2.number_input('Number of Recommendations', min_value=1, max_value=50, value=10)
